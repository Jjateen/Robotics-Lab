# This Python progtam aims at implementing a controller based on the REINFORCE Policy Gradient Approach to solve the CartPole Problem
...
while not solved and episodeCount < episodeLimit:
    ...
    for step in range(env.stepsPerEpisode):
        ...
        agent.episode_buffer.append([observation,selectedAction,reward])
        env.episodeScore += reward
        if done:
            env.episodeScoreList.append(env.episodeScore)
            agent.update_network(agent.episode_buffer)
            solved = env.solved()
            break
